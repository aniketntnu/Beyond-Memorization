...
class CrossAttention(nn.Module):
    ...
    def shuffle_context(self, context):
        #print("\n\t shuffling!!:",context.shape)
        # Check if shuffle indices need to be initialized or resized for the current context size
        if 1:#self.shuffle_indices is None or self.shuffle_indices.size(0) != context.size(1):
            # Generate a fixed permutation for the current sequence length of context
            self.shuffle_indices = torch.randperm(context.size(0), device=context.device)
        
        # Apply the permutation to context
        #return context[:, self.shuffle_indices, :]

        return context[self.shuffle_indices,:, :]


    def select_and_concat_context(self, context):
        batch_size, seq_len, feature_dim = context.size()
        
        # Randomly select `num_select` distinct images from the batch
        selected_indices = torch.randperm(batch_size)[:self.num_select]
        selected_context = context[selected_indices]  # Shape: [num_select, seq_len, feature_dim]
        
        #print("\n\t context.shape:",context.shape,"\n selected_context.shape:",selected_context.shape,"\n self.num_select:",self.num_select)
        # Concatenate selected images along the sequence dimension
        concatenated_context = selected_context.view(1, self.num_select * seq_len, feature_dim)
        
        #print("\n\t 1.concatenated_context.shape =",concatenated_context.shape)        
        # Expand to match batch size for broadcasting (same concatenated context for each batch element)
        concatenated_context = concatenated_context.expand(batch_size, -1, -1)
        
        #print("\n\t 2.concatenated_context.shape =",concatenated_context.shape)        

        #print("\n\t concatenated_context.shape:=",concatenated_context.shape)
        
        return concatenated_context

    
    def attenMapsReturn(self, attn):
    ...
    def forward(self, x, context=None, mask=None):
        ...
        if self.args.batchCrossAttention and context.size() == x.size():# and self.args.selfAttentionShuffle:

            #print("\n\t 1.context.shape:",context.shape," x.shape:",x.shape)

                        
            context = self.shuffle_context(context)  # Shuffle context to break cell potential
            #print("\n\t 11.context.shape:",context.shape)

            context = self.select_and_concat_context(context)

            #print("\n\t 2.context.shape:",context.shape," x.shape:",x.shape)        
        ...
        # Modify attention map for odd indices if x and context have different sizes
        if x.size() != context.size() and self.args.spatialCross:
            # Create a mask to select even and odd indices
            even_indices = torch.arange(0, sim.size(0), 2, device=sim.device)
            odd_indices = even_indices + 1
            
            # Ensure odd_indices are within bounds
            odd_indices = odd_indices[odd_indices < sim.size(0)]
            
            # Copy attention maps for odd indices from the previous even index
            sim[odd_indices] = sim[even_indices]
            #sim[even_indices] = sim[odd_indices]        
        ...
        #attn = self.attenMapsReturn(attn)
...
class BasicTransformerBlock(nn.Module):
    def __init__(self,args, dim, n_heads, d_head, dropout=0., context_dim=None, gated_ff=True, checkpoint=True):
        ...
        self.attn1 = CrossAttention(args,query_dim=dim, heads=n_heads, dim_head=d_head, dropout=dropout)  # is a self-attention for the image
        self.attnc = CrossAttention(args,query_dim=dim, heads=n_heads, dim_head=d_head, dropout=dropout)  # is a self-attention for the context
        self.ff = FeedForward(dim, dropout=dropout, glu=gated_ff)
        self.attn2 = CrossAttention(args,query_dim=dim, context_dim=context_dim,
    ...
    def forward(self, x, context=None):
        ...
        x, attn1, attn2 = checkpoint(self._forward, (x, context), self.parameters(), self.checkpoint)
        return x
...
class SpatialTransformer(nn.Module):
    ...
    def forward(self, x, context=None):
        ...
        for block in self.transformer_blocks:
            ...
            x,attn1, attn2 = block(x, context=context)
...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    def forward(self, x, emb, context=None):
        ...
        attn2_maps = []  # List to collect all attention maps from SpatialTransformer layers
            ...
            for _ in range(num_res_blocks):
                ...
                if ds in attention_resolutions:
                    ...
                    layers.append(
                        ...
                        ) if not use_spatial_transformer else SpatialTransformer(args,
