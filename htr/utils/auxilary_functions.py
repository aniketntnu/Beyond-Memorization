...
def image_resize(img,img_path, height=None, width=None):
    ...
    try:
        ...
        print("\n\t output_shape=",(height, width)," img_path:",img_path)
        #input("check")    
...
def centered(word_img, tsize, centering=(.5, .5), border_value=None):
    ...
    #print("\n\t word_img.shape:",word_img.shape)
    
    if word_img.ndim == 3:
        word_img = word_img[:,:,0]
    
    
    #print("\n\t Before padding word_img.shape:",word_img.shape)

    word_img = np.pad(word_img[ys:ye, xs:xe], (padh, padw), 'constant', constant_values=border_value)
    #print("\n\t after padding word_img.shape:",word_img.shape)
    
    #from PIL import Image
   
    #img = Image.fromarray(word_img)
    #img.save("/home/aniketag/Documents/phd/TensorFlow-2.x-YOLOv3_simula/Handwriting-1-master/PapersReimplementations/WordStylist/wordStyleOutPut/HTR-best-practices/savedImages/aaaa.png")
    #input("check!!")
    return word_img