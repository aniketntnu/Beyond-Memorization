...
class WordLineDataset(Dataset):
    ...
    '''
    def __init__(self, 
        args,basefolder: str = 'datasets/',                #Root folder
    ...
    def __finalize__(self):
        ...
        save_file = '/media/aniketag/dc99d394-6b53-4f0f-a64c-240aa989dec1/dataSets/saved_models/{}_{}_{}.pt'.format(self.subset, self.segmentation_level, self.setname) #dataset_path + '/' + set + '_' + level + '_IAM.pt'

        if isfile(save_file) is False:
            
            print("\n\t calling from word_dataset.py ")
    ...
    def __getitem__(self, index):
        ...
        img = image_resize(img,img_path, height=int(1.0 * nheight), width=int(1.0 * nwidth))


        #print("\n\ 00:",img.shape)
        
        if self.args.saveLogs:
            img_uint888 = (img * 255).astype(np.uint8)
            img_io.imsave( savedOcrImages+str(index)+"_2.png", img_uint888)

        img = centered(img, (fheight, fwidth), border_value=0.0) # ((32, 128, 3), (128, 1024))


        if self.args.saveLogs:
            img_uint888 = (img * 255).astype(np.uint8)
            #print("\n\ 00:",img.shape)
            img_io.imsave( savedOcrImages+str(index)+"_33.png", img_uint888)
        ...
        #img_io.imsave( savedOcrImages+str(index)+"_333.png", img_uint888)
        ...
        if self.args.saveLogs:
            ...
            img1 = np.stack([img1,img1,img1],2).squeeze()
            img_io.imsave( savedOcrImages+str(index)+"_333.png", img1)
